...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--cases", type=Path, default=Path("tests/schema_rag_cases.json"))
    parser.add_argument("--report", type=Path, default=Path("reports/schema-rag-evaluation.json"))
...
async def main() -> int:
    ...
    permission = container.permissions.for_principal(Principal("u_demo_user", ("USER",)))
    ...
    try:
        for case in cases:
            ...
            field_recall = (
                len(expected_fields & observed_fields) / len(expected_fields)
                if expected_fields
                else 1.0
            )
            ...
            outcomes.append(
                {
                    ...
                    "object_count": len(observed_objects),
        ...
        report = {
            ...
            "metrics": {
                ...
                "context_precision": round(statistics.mean(precisions), 6) if precisions else 0,
                "p95_context_tokens": percentile(tokens, 0.95),
        ...
        print(
            f"evaluated {len(outcomes)} schema-linking cases; "
