...
def _tokens(value: object) -> int:
    return max(1, (len(json.dumps(
        value, ensure_ascii=False, separators=(",", ":"), default=str)) + 3) // 4)


def _fuse(dense: list[dict[str, Any]], lexical: list[dict[str, Any]], *,
          dense_weight: float = 0.6, lexical_weight: float = 0.4,
    ...
    ranked = sorted(entries.values(), key=lambda item: (
        -float(item["score"]), str(item["kind"]), str(item["target_id"])))[:limit]
...
class ContextBudgeter:
    ...
    def apply(self, *, objects: list[CatalogObject], fields: list[CatalogField],
              joins: list[JoinPath], metrics: list[str],
              required_fields: set[str]) -> tuple[
                  list[CatalogObject], list[CatalogField], list[JoinPath], int]:
        ...
        for item in objects:
            ...
            candidates.sort(key=lambda field: (
                0 if field.name in required_fields else
                1 if field.classification == "BUSINESS_TIME" else
                2 if field.classification == "IDENTIFIER" else 3,
            ...
            selected_fields.extend(required + optional[:max(
                0, self.max_fields_per_object - len(required))])

        def payload() -> dict[str, Any]:
            return {"objects": [item.model_dump() for item in objects],
        ...
        if _tokens(payload()) > self.max_tokens:
            selected_fields = [field.model_copy(update={"aliases": []})
...
class LLMReranker:
    ...
    async def rerank(self, query: str, objects: list[CatalogObject]) -> tuple[
        ...
        draft, trace = await self.llm.structured(
            system=("Rank catalog object IDs by relevance to the user question. "
                    "Return every candidate ID exactly once, most relevant first. "
                    "Do not add IDs."),
            user=json.dumps({"question": query, "candidates": [{
                "object_id": item.object_id, "name": item.name,
                "grain": item.grain, "domain": item.domain,
            } for item in objects]}, ensure_ascii=False),
            schema=_RerankResult, purpose="reranker", temperature=0.0,
            prompt_version="catalog_rerank_v1")
        expected = {item.object_id for item in objects}
        returned = list(dict.fromkeys(draft.ranking))
        unknown = set(returned) - expected
        if unknown:
            raise RuntimeAgentError("RAG_RERANK_FAILED",
                                    "reranker returned unauthorized candidate IDs",
                                    details={"unknown_candidate_count": len(unknown)})
        ...
        return ranking, asdict(trace) | {
            "purpose": "reranker", "completed_missing_ids": len(missing)}
    ...
    async def retrieve(
        ...
        if not permission.allowed_source_ids:
            raise RuntimeAgentError("PERMISSION_DENIED",
                                    "no authorized catalog source is available")
        query = schema_gap.narrow_query if schema_gap else task.question
        version = await self._thread(self.repository.version,
        ...
        dense = await self._thread(
            self.index.search, manifest, layer="object", embedding=embedding,
            source_ids=selected_sources, limit=self.max_objects * 4,
            object_ids=restricted_ids)
        lexical = await self._thread(
            self.repository.lexical_search, query, scoped_permission,
            self.max_objects * 4, layers=("object",), object_ids=restricted_ids)
        fused = _fuse(dense, lexical, dense_weight=self.dense_weight,
        ...
        if metric_hits and float(metric_hits[0]["score"]) >= self.min_score:
            metric_gap = (1.0 if len(metric_hits) == 1 else
                          float(metric_hits[0]["score"]) - float(metric_hits[1]["score"]))
        ...
        metric_ids, dimension_ids, required_tables, required_fields = await self._thread(
            self.repository.semantic_bindings, query, scoped_permission,
            dense_metric_ids=dense_metric_ids)
        object_hits = [item for item in fused if item["kind"] == "object"]
        seed_ids = [str(item["target_id"]) for item in object_hits
                    if float(item["score"]) >= self.min_score]
        object_ids = await self._thread(
            self.repository.expand_object_ids, seed_ids, scoped_permission,
            self.max_join_hops, self.max_objects, required_names=required_tables)
        objects, _, _, version = await self._thread(
            self.repository.hydrate, object_ids, scoped_permission)
        object_scores = {str(item["target_id"]): float(item["score"])
                         for item in object_hits}
        objects = [item.model_copy(update={"score": max(
            self.min_score if item.name in required_tables else 0,
            object_scores.get(item.object_id, 0.0))}) for item in objects]
        ranking, reranker_trace = await self.reranker.rerank(query, objects)
        ...
        field_dense = await self._thread(
            ...
            source_ids=selected_sources, object_ids=object_ids,
            limit=max(1, self.max_objects * self.max_fields), kinds=["field"],
            denied_classifications=permission.denied_classifications)
        field_lexical = await self._thread(
            self.repository.lexical_search, query, scoped_permission,
            max(1, self.max_objects * self.max_fields), layers=("field_entity",),
            object_ids=object_ids)
        field_hits = _fuse(
            field_dense, field_lexical, dense_weight=self.dense_weight,
            lexical_weight=1 - self.dense_weight,
        ...
        retrieval_trace = {
            ...
            "catalog_version": version, "index_version": manifest.index_version,
