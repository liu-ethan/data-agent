...
RECORDS = (
    CatalogRecord("obj_shops", "shops", "shop", "ECOMMERCE_TRADE", ("店铺", "门店", "商店"),
        (("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)), ("shop_name", "VARCHAR", "BUSINESS", ("店铺名",)),
         ("region_code", "VARCHAR", "BUSINESS", ("地区编码",)), ("region_name", "VARCHAR", "BUSINESS", ("地区", "区域")),
         ("status", "VARCHAR", "STATUS", ("状态",)))),
    CatalogRecord("obj_users", "users", "buyer", "ECOMMERCE_TRADE", ("用户", "买家"),
        (("user_id", "VARCHAR", "IDENTIFIER", ("买家ID",)), ("phone", "VARCHAR", "PHONE", ("手机号",)),
         ("id_number", "VARCHAR", "ID_CARD", ("身份证",)), ("created_at", "DATETIME", "BUSINESS_TIME", ("注册时间",)))),
    CatalogRecord("obj_categories", "categories", "category", "ECOMMERCE_TRADE", ("品类", "类目", "分类"),
        (("category_id", "VARCHAR", "IDENTIFIER", ("品类ID",)), ("parent_id", "VARCHAR", "IDENTIFIER", ("父品类",)),
         ("category_name", "VARCHAR", "BUSINESS", ("品类名", "分类名称")))),
    CatalogRecord("obj_products", "products", "product", "ECOMMERCE_TRADE", ("商品", "产品"),
        (("product_id", "VARCHAR", "IDENTIFIER", ("商品ID",)), ("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)),
         ("category_id", "VARCHAR", "IDENTIFIER", ("品类ID",)), ("product_name", "VARCHAR", "BUSINESS", ("商品名",)),
         ("status", "VARCHAR", "STATUS", ("商品状态",)))),
    CatalogRecord("obj_orders", "orders", "order", "ECOMMERCE_TRADE", ("订单", "交易订单"),
        (("order_id", "VARCHAR", "IDENTIFIER", ("订单ID",)), ("user_id", "VARCHAR", "IDENTIFIER", ("买家ID",)),
         ("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)), ("status", "VARCHAR", "STATUS", ("订单状态",)),
         ("paid_at", "DATETIME", "BUSINESS_TIME", ("支付时间",)), ("pay_amount", "DECIMAL", "AMOUNT", ("支付金额",)),
         ("created_at", "DATETIME", "BUSINESS_TIME", ("下单时间",)))),
    CatalogRecord("obj_order_items", "order_items", "order_item", "ECOMMERCE_TRADE", ("订单明细", "商品明细", "订单商品"),
        (("item_id", "VARCHAR", "IDENTIFIER", ("明细ID",)), ("order_id", "VARCHAR", "IDENTIFIER", ("订单ID",)),
         ("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)), ("product_id", "VARCHAR", "IDENTIFIER", ("商品ID",)),
         ("quantity", "INT", "MEASURE", ("数量",)), ("item_paid_amount", "DECIMAL", "AMOUNT", ("商品实付金额",)))),
    CatalogRecord("obj_refunds", "refunds", "refund", "ECOMMERCE_TRADE", ("退款", "退货"),
        (("refund_id", "VARCHAR", "IDENTIFIER", ("退款ID",)), ("order_id", "VARCHAR", "IDENTIFIER", ("订单ID",)),
         ("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)), ("status", "VARCHAR", "STATUS", ("退款状态",)),
         ("refund_amount", "DECIMAL", "AMOUNT", ("退款金额",)), ("refunded_at", "DATETIME", "BUSINESS_TIME", ("退款时间",)))),
    CatalogRecord("obj_refund_items", "refund_items", "refund_item", "ECOMMERCE_TRADE", ("退款明细",),
        (("refund_item_id", "VARCHAR", "IDENTIFIER", ("退款明细ID",)), ("refund_id", "VARCHAR", "IDENTIFIER", ("退款ID",)),
         ("shop_id", "VARCHAR", "IDENTIFIER", ("店铺ID",)), ("order_item_id", "VARCHAR", "IDENTIFIER", ("订单明细ID",)),
         ("refund_amount", "DECIMAL", "AMOUNT", ("退款明细金额",)))),
)

METRICS = {
    "gmv": {"name": "支付 GMV", "formula": "SUM(order_items.item_paid_amount)",
            "time_field": "orders.paid_at", "required_filters": ["orders.status = PAID"]},
    "paid_order_count": {"name": "支付订单数", "formula": "COUNT(DISTINCT orders.order_id)",
            "time_field": "orders.paid_at", "required_filters": ["orders.status = PAID"]},
    "paid_buyer_count": {"name": "支付买家数", "formula": "COUNT(DISTINCT orders.user_id)",
            "time_field": "orders.paid_at", "required_filters": ["orders.status = PAID"]},
    "average_order_value": {"name": "客单价", "formula": "gmv / paid_order_count",
            "time_field": "orders.paid_at", "required_filters": ["orders.status = PAID"]},
    "refund_amount": {"name": "退款金额", "formula": "SUM(refunds.refund_amount)",
            "time_field": "refunds.refunded_at", "required_filters": ["refunds.status = SUCCESS"]},
    "refund_rate": {"name": "金额退款率", "formula": "refund_amount / gmv",
            "time_field": "refunds.refunded_at", "required_filters": ["refunds.status = SUCCESS"]},
    "category_gmv": {"name": "品类 GMV", "formula": "SUM(order_items.item_paid_amount)",
            "time_field": "orders.paid_at", "required_filters": ["orders.status = PAID"]},
}

JOINS = (
    JoinPath(join_id="orders_to_order_items", left="orders.order_id", right="order_items.order_id", cardinality="one_to_many"),
    JoinPath(join_id="order_items_to_products", left="order_items.product_id", right="products.product_id", cardinality="many_to_one"),
    JoinPath(join_id="products_to_categories", left="products.category_id", right="categories.category_id", cardinality="many_to_one"),
    JoinPath(join_id="orders_to_shops", left="orders.shop_id", right="shops.shop_id", cardinality="many_to_one"),
    JoinPath(join_id="orders_to_users", left="orders.user_id", right="users.user_id", cardinality="many_to_one"),
    JoinPath(join_id="orders_to_refunds", left="orders.order_id", right="refunds.order_id", cardinality="one_to_many"),
    JoinPath(join_id="refunds_to_refund_items", left="refunds.refund_id", right="refund_items.refund_id", cardinality="one_to_many"),
)
...
class CatalogRetrievalService:
    ...
    def __init__(self, *, catalog_version: str = CATALOG_VERSION, max_objects: int = 5,
                 max_fields: int = 8, max_tokens: int = 3000, min_score: float = 0.55,
                 ambiguity_gap: float = 0.08, records: tuple[CatalogRecord, ...] = RECORDS) -> None:
    ...
    def retrieve(self, task: TaskFrame, permission: PermissionContext,
        ...
        requested_metrics = self._metrics(task.question)
        required_names: set[str] = set()
        if requested_metrics:
            required_names.update({"orders", "order_items"})
        if any(metric in {"gmv", "category_gmv", "paid_order_count", "paid_buyer_count"} for metric in requested_metrics):
            required_names.update({"products", "categories"} if "category_gmv" in requested_metrics else set())
        if any(metric in {"refund_amount", "refund_rate"} for metric in requested_metrics):
            required_names.add("refunds")
        for record in self.records:
            if record.name == "users" and any(c in permission.denied_classifications for c in ("PHONE", "ID_CARD")):
                pass
            text = " ".join((record.name, record.grain, record.domain, *record.aliases,
            ...
            if score >= self.min_score or record.name in query.lower() or alias_match or record.name in required_names:
                if record.name in required_names:
                    score = max(score, 0.82)
                candidates.append((record, score))
        candidates.sort(key=lambda pair: pair[1], reverse=True)
        candidates = candidates[:self.max_objects]
        objects = [CatalogObject(object_id=r.object_id, name=r.name, grain=r.grain,
        ...
        for record, score in candidates:
            ...
            for field_name, dtype, classification, aliases in record.fields:
                ...
                if count >= self.max_fields:
                    break
                fields.append(CatalogField(field_id=f"field_{record.name}_{field_name}",
                    name=f"{record.name}.{field_name}", data_type=dtype,
                    classification=classification, aliases=list(aliases), score=round(score, 4),
                    object_id=record.object_id, nullable=classification not in {"IDENTIFIER", "BUSINESS_TIME"},
                    permission_policy_version=permission.policy_version,
                    retrieval_method=self.retrieval_method))
                count += 1
        metrics = requested_metrics
        joins = [join for join in JOINS if join.left.split(".")[0] in {r.name for r, _ in candidates}
        ...
        ambiguous: list[str] = []
        if len(candidates) >= 2 and candidates[0][1] - candidates[1][1] < self.ambiguity_gap and not metrics:
        ...
        if sensitive_request and permission.denied_classifications:
            ...
            status = CoverageStatus.SUFFICIENT if not missing and not ambiguous and objects else CoverageStatus.PARTIAL
        gap = None
        if status != CoverageStatus.SUFFICIENT:
            gap = SchemaGap(gap_id=f"gap_{uuid4().hex[:12]}", missing_concepts=missing or ambiguous,
                candidate_object_ids=[o.object_id for o in objects], narrow_query=query,
                reason="catalog evidence is incomplete or ambiguous",
                retrieval_round=(schema_gap.retrieval_round + 1 if schema_gap else 1))
        payload = {"objects": [o.model_dump() for o in objects], "fields": [f.model_dump() for f in fields],
                   "metrics": metrics, "join_paths": [j.model_dump() for j in joins], "coverage": status}
        ...
        context = GroundedContext(context_id=existing_context_id or f"ctx_{uuid4().hex[:12]}",
            ...
            permission_policy_version=permission.policy_version)
        result = CoverageResult(status=status, covered=covered, missing=missing,
            ambiguous=ambiguous, confidence_notes=["deterministic baseline retrieval"], schema_gap=gap)
    ...
    def _metrics(question: str) -> list[str]:
        ...
        if any(term in q for term in ("gmv", "销售额", "销售", "成交额")):
            matches.append("category_gmv" if any(term in question for term in ("品类", "类目")) else "gmv")
...
def build_permission(user_id: str, config: dict) -> PermissionContext:
    ...
    if not item:
        return PermissionContext(user_id=user_id, roles=[], scope_mode="NONE",
            policy_version=config.get("default_policy_version", "policy_unknown"))
    role = item.get("role", "USER")
    shops = list(item.get("shop_ids", []))
    return PermissionContext(user_id=user_id, roles=[role], scope_mode="ALLOWLIST" if shops else "NONE",
        allowed_shop_ids=shops, denied_classifications=list(config.get("denied_classifications", [])),
        policy_version=config.get("default_policy_version", "policy_local_v1"))


def generate_synthetic_metadata(source_count: int = 100, object_count: int = 1000,
                                fields_per_object: int = 30) -> tuple[CatalogRecord, ...]:
    ...
    for index in range(object_count):
        ...
        fields = tuple((f"field_{field:02d}", "VARCHAR" if field % 3 else "DATETIME",
                        "BUSINESS_TIME" if field % 3 == 0 else "BUSINESS",
                        (f"字段{field:02d}",)) for field in range(fields_per_object))
        records.append(CatalogRecord(f"syn_obj_{index:04d}", name, "synthetic", "NOISE_DOMAIN",
            (f"table{index:04d}", f"业务干扰{index:04d}"), fields,
