...
class MySQLCatalogRepository:
    ...
    def synchronize(self, snapshot: CatalogSnapshot) -> CatalogSnapshot:
        ...
        with self.engine.begin() as connection:
            metrics = [
                ...
                for row in connection.execute(
                    text(
                        ...
                        "FROM metric_definitions ORDER BY metric_id"
                    )
                )
                .mappings()
                .all()
            ...
            aliases = [
                ...
                for row in connection.execute(
                    text(
                        ...
                        "FROM entity_aliases ORDER BY alias_id"
                    )
                )
                .mappings()
                .all()
            ...
            object_ids = {str(row["object_name"]): str(row["object_id"]) for row in old_objects}
            ...
            if stale_objects:
                ...
                connection.execute(
                    text(f"DELETE FROM catalog_fields WHERE object_id IN ({clause})"), values
                )
                connection.execute(
                    text(f"DELETE FROM catalog_objects WHERE object_id IN ({clause})"), values
                )
            ...
            for table_name, object_id in selected_object_ids.items():
                ...
                existing_ids = {str(row["field_name"]): str(row["field_id"]) for row in existing}
                current_names: set[str] = set()
                for item in (field for field in snapshot.fields if field.table_name == table_name):
                ...
                if stale_fields:
                    ...
                    connection.execute(
                        text(f"DELETE FROM catalog_fields WHERE field_id IN ({clause})"), values
                    )
            ...
            if old_physical:
                ...
                connection.execute(
                    text(f"DELETE FROM table_relations WHERE relation_id IN ({physical_clause})"),
                    physical_params,
                )
            ...
            connection.execute(
                text("DELETE FROM catalog_metric_sources WHERE source_id=:source_id"),
                {"source_id": snapshot.source_id},
            )
            connection.execute(
                text(
            ...
            for relation in curated_relations:
                ...
                if left_table in table_names and right_table in table_names:
                    ...
                    """),
                        {"relation_id": relation["relation_id"], "source_id": snapshot.source_id},
                    )
            for metric in metrics:
                references = self._references(
                    " ".join(
                        str(metric.get(key) or "")
                        for key in ("formula", "time_field", "required_filters_json")
                    )
                )
                if references & table_names:
                    ...
                    """),
                        {"metric_id": metric["metric_id"], "source_id": snapshot.source_id},
                    )
            ...
            all_relations = list(
                {
                    ...
                    for item in [*curated_relations, *physical_relations]
                }.values()
            )
            documents = self._documents(
                snapshot, selected_object_ids, selected_field_ids, metrics, aliases, all_relations
            )
    ...
    def version(self, source_ids: list[str]) -> str:
        ...
        if len(rows) != len(set(source_ids)):
            raise RuntimeAgentError(
                "CATALOG_VERSION_MISMATCH", "an authorized catalog source is missing"
            )
        versions = {str(row["catalog_version"]) for row in rows}
        if len(versions) != 1:
            raise RuntimeAgentError(
                "CATALOG_VERSION_MISMATCH", "catalog sources have incompatible versions"
            )
        return versions.pop()
    ...
    def lexical_search(
        ...
        *,
        ...
        if permission.denied_classifications:
            denied_clause, denied_params = _params("denied", permission.denied_classifications)
        ...
        scope = (
            f"d.source_id IN ({source_clause}) AND d.catalog_version=:version "
            ...
            corpus = (
                connection.execute(
                    text(
                        ...
                        f"FROM catalog_search_documents d WHERE {scope}"
                    ),
                    params,
                )
                .mappings()
        ...
        return self._bm25(
            ...
            {str(row["term"]): int(row["df"]) for row in dfs},
            limit,
        )

    def hydrate(
        self, ids: list[str], permission: PermissionContext, *, field_ids: list[str] | None = None
    ) -> tuple[list[CatalogObject], list[CatalogField], list[JoinPath], str]:
        if not ids:
        ...
        field_scores = {
            field_id: 1 - index / max(1, len(field_ids or []))
        ...
        output_fields = [
            CatalogField(
                ...
                + aliases_by_target.get(f"{names[str(row['object_id'])]}.{row['field_name']}", []),
            ...
            for row in fields
            if str(row["classification"]) not in denied
        ]
        ...
        for row in relations:
            ...
            if left.split(".")[0] not in object_names or right.split(".")[0] not in object_names:
                continue
    ...
    def expand_object_ids(
        ...
        required = [by_name[name] for name in sorted(required_names or set()) if name in by_name]
    ...
    def semantic_bindings(
        ...
        query: str,
        permission: PermissionContext,
        dense_metric_ids: list[str] | None = None,
        proposed_metric_ids: list[str] | None = None,
        proposed_dimension_ids: list[str] | None = None,
        ...
        with self.engine.connect() as connection:
            ...
            aliases = (
                connection.execute(
                    text("SELECT alias_text,target_type,target_id FROM entity_aliases")
                )
                .mappings()
                .all()
            )
        ...
        metric_ids = [
            metric_id
            for metric_id, row in available.items()
            if metric_id.lower() in lowered or str(row["name"]).lower() in lowered
        ]
        metric_ids = [
            *(item for item in (proposed_metric_ids or []) if item in available),
            *metric_ids,
        ]
        if not metric_ids:
            metric_ids.extend(item for item in (dense_metric_ids or [])[:1] if item in available)
        dimensions: list[str] = []
        for row in aliases:
        ...
        dimensions = [
            *(item for item in (proposed_dimension_ids or []) if item in allowed_dimension_refs),
            *dimensions,
        ]
        metric_ids = list(dict.fromkeys(metric_ids))
        required_tables: set[str] = set()
        required_fields: set[str] = set(dimensions)
        required_tables.update(item.split(".", 1)[0] for item in dimensions if "." in item)
        for metric_id in metric_ids:
            row = available[metric_id]
            refs = self._qualified_references(
                " ".join(
                    str(row[key] or "")
                    for key in ("formula", "time_field", "required_filters_json")
                )
    ...
    def active_manifest(self, catalog_version: str | None = None) -> IndexManifest:
        ...
        if catalog_version:
            where += " AND catalog_version=:version"
            params["version"] = catalog_version
        with self.engine.connect() as connection:
            row = (
                connection.execute(
                    text(f"""
        ...
        if not row:
            raise RuntimeAgentError("RAG_INDEX_MISSING", "no active Milvus index manifest exists")
    ...
    def activate_manifest(
        ...
        *,
        ...
        with self.engine.begin() as connection:
            connection.execute(
                text("UPDATE catalog_index_manifests SET status='RETIRED' WHERE status='ACTIVE'")
            )
            connection.execute(
                text("""
            ...
            """),
                {
                    ...
                    "counts": json.dumps(document_counts, sort_keys=True),
                    "now": now,
                },
            )
    ...
    def _references(value: str) -> set[str]:
        return {
            item.split(".")[0]
            for item in re.findall(r"[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*", value)
        }

    @staticmethod
    def _qualified_references(value: str) -> set[str]:
        return set(re.findall(r"[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*", value))
    ...
    def _bm25(
        ...
        return [metadata[item] | {"score": round(scores[item] / maximum, 6)} for item in ranked]
    ...
    def _documents(
        ...
        for item in snapshot.objects:
            ...
            add(
                ...
                f"{' '.join(aliases_by_target[object_id])}",
                object_id,
            )
        for item in snapshot.fields:
    ...
    def _replace_documents(
        connection: Any, source_id: str, documents: list[SearchDocument]
    ) -> None:
        connection.execute(
            text("DELETE FROM catalog_search_documents WHERE source_id=:source_id"),
            {"source_id": source_id},
        )
        for item in documents:
            ...
            term_rows = [
                {"document_id": item.document_id, "term": term, "frequency": frequency}
