...
async def agent_node(runtime: Any, run: dict[str, Any]) -> dict[str, Any]:
    ...
    await emit_event(runtime, run, "node.started", node="agent_node", action=state.next_action)
    ...
    if iterations > runtime.max_iterations:
    ...
    elif reason := forbidden_request(message, run.get("permission")):
        ...
        state.next_action = (
            Action.RETRIEVE if used < runtime.max_retrieval_rounds else Action.ASK_USER
        )
        ...
        state.latest_observation.status in {ResultStatus.REJECTED, ResultStatus.FAILED}
        ...
        state.budgets["query_retries_used"] = int(state.budgets.get("query_retries_used", 0)) + 1
