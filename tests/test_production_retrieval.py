...
MANIFEST = IndexManifest(
    ...
    collections={layer: layer for layer in ("source_domain", "object", "field_entity", "relation")},
    document_counts={layer: 1 for layer in ("source_domain", "object", "field_entity", "relation")},
)
...
class FakeRepository:
    ...
    def semantic_bindings(self, query, permission, dense_metric_ids=None, **_kwargs):
    ...
    def expand_object_ids(self, seed_ids, permission, max_hops, max_objects, required_names=None):
...
def permission():
    return PermissionContext(
        ...
        allowed_shop_ids=["shop_1"],
        allowed_source_ids=["source_allowed"],
...
def test_production_retrieval_is_permission_first_and_returns_bounded_evidence():
    ...
    task = TaskFrame(task_id="t", user_id="u", question="昨天 GMV", intent="DATA_QUERY")
...
def test_context_budget_never_drops_required_fields():
    ...
    _, selected, _, count = ContextBudgeter(700, 4).apply(
        objects=[obj], fields=fields, joins=[], metrics=["gmv"], required_fields={"orders.field_19"}
    )
...
def test_retrieval_replaces_model_proposals_with_authoritative_bindings():
    service = ProductionCatalogRetrievalService(
        FakeRepository(), FakeIndex(), FakeEmbedder(), FakeReranker(), max_tokens=3000
    )
    ...
    asyncio.run(
        retrieval_node(
            Runtime(), {"state": state, "permission": permission(), "checkpoint_version": -1}
        )
    )
...
def test_reranker_deterministically_completes_missing_authorized_ids():
    ...
    objects, _, _, _ = FakeRepository().hydrate(["obj_orders", "obj_items"], permission())
...
def test_reranker_drops_unknown_candidate_ids():
    @dataclass(frozen=True)
    class Trace:
        model: str = "reranker"
    ...
    class InvalidLLM:
        async def structured(self, schema, **kwargs):
            return schema(ranking=["obj_forbidden", "obj_orders"]), Trace()

    objects, _, _, _ = FakeRepository().hydrate(["obj_orders"], permission())
    ranking, trace = asyncio.run(LLMReranker(InvalidLLM()).rerank("GMV", objects))
    assert ranking[0] == "obj_orders"
    assert "obj_forbidden" not in ranking
    assert trace["dropped_unknown_ids"] == 1


def test_retrieval_falls_back_when_reranker_llm_is_invalid():
    class BrokenReranker:
        async def rerank(self, query, objects):
            raise RuntimeAgentError("LLM_RESPONSE_INVALID", "reranker json")

    service = ProductionCatalogRetrievalService(
        FakeRepository(), FakeIndex(), FakeEmbedder(), BrokenReranker()
    )
    ...
    assert {item.object_id for item in context.objects} == {"obj_orders", "obj_items"}
    assert coverage.status.value in {"SUFFICIENT", "PARTIAL"}
    assert context.model_traces[-1]["fallback"] == "permission_filtered_order"
