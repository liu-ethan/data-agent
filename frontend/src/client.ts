...
export function newClientRequestId():string{
  ...
  // Idempotency identifiers are not credentials. This fallback keeps the UI
  // functional on LAN HTTP origins where Web Crypto may be unavailable.
...
export async function requestJson<T>(path:string,token?:string,init:RequestInit={}):Promise<T>{
  ...
  if(!response.ok){const code=String(payload.error_code??payload.detail??'REQUEST_FAILED');throw new ApiError(code,payload.trace_id,response.status)}
  return payload as T
}

export function tokenExpiry(token:string):Date|undefined{
  try{const part=token.split('.')[1];if(!part)return;const normalized=part.replace(/-/g,'+').replace(/_/g,'/');const payload=JSON.parse(atob(normalized)) as {exp?:number};return payload.exp?new Date(payload.exp*1000):undefined}catch{return undefined}
}
...
export async function consumeSse(args:{url:string;token:string;requestId:string;cursor:{lastEventId:number};signal:AbortSignal;onEvent:(event:StreamEvent)=>void|Promise<void>}):Promise<void>{
  const headers:Record<string,string>={Authorization:`Bearer ${args.token}`,'X-Request-ID':args.requestId}
  if(args.cursor.lastEventId)headers['Last-Event-ID']=String(args.cursor.lastEventId)
  const response=await fetch(args.url,{headers,signal:args.signal})
  if(!response.ok||!response.body){
    const payload=await response.json().catch(()=>({detail:response.status===401?'AUTH_INVALID':'STREAM_UNAVAILABLE'}))
  ...
  async function processPacket(packet:string){
    ...
    const lines=packet.split('\n'),eventId=lines.find(line=>line.startsWith('id:'))?.slice(3).trim(),name=lines.find(line=>line.startsWith('event:'))?.slice(6).trim()
    const data=lines.filter(line=>line.startsWith('data:')).map(line=>line.slice(5).trimStart()).join('\n');if(!name||!data)return
    const parsed=JSON.parse(data) as StreamEvent;parsed.event=name as StreamEvent['event']
    if(eventId){parsed.eventId=Number(eventId);args.cursor.lastEventId=parsed.eventId}
    terminal=['run.completed','run.failed','interrupt.created'].includes(name)
    await args.onEvent(parsed)
  }
  for(;;){
    const{done,value}=await reader.read();if(done){if(buffer.trim())await processPacket(buffer.replace(/\r\n/g,'\n').trimEnd());break}
    buffer+=decoder.decode(value,{stream:true});const packets=buffer.replace(/\r\n/g,'\n').split('\n\n');buffer=packets.pop()??''
