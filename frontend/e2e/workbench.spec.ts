...
test('desktop analyst can query, inspect thinking and download governed results',async({page})=>{
  await mockWorkbench(page);await login(page)
  await page.getByRole('textbox',{name:'问题'}).fill('昨天各品类 GMV');await page.getByRole('button',{name:'发送'}).click()
  await expect(page.getByText('查询完成，共 51 行。')).toBeVisible();await expect(page.getByRole('cell',{name:'1280'})).toBeVisible()
  await page.getByText('思考过程').click();await expect(page.getByText(/21 ms/)).toBeVisible()
...
test('clarification interrupt resumes exactly from the visible choice',async({page})=>{
  ...
  await page.getByRole('textbox',{name:'问题'}).fill('退款率');await page.getByRole('button',{name:'发送'}).click();await expect(page.getByText('退款率使用哪个口径？')).toBeVisible();await page.getByRole('button',{name:'金额退款率'}).click();await expect(page.getByText('已按金额退款率计算。')).toBeVisible()
})
